...
unet_paths = os.listdir(config['unet_testing_path'])
print(unet_paths)

for unet_path in unet_paths:

    print('TESTING UNET:', unet_path)
    unet_path = os.path.join(config['unet_testing_path'], unet_path)
    ...
    for run in range(0, number_runs):
        ...
        #current_model = unet_path + '/checkpoints/experiments/exp1/models/unet_' + str(run) + '.h5'
    ...
    metric_dict = {'Precision': Precision, 'Recall' : Recall, 'mAP': AP, 'mAP_func (AUC)': mAP_func}

    dumped = json.dumps(metric_dict, cls=NumpyEncoder)
    with open(output_folder + '/metrics.txt', 'w') as file:

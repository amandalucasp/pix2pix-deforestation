...
def load_patches(root_path, folder):
  ...
  for i in range(len(img_files)):
    ...
    # print(patches[-1].shape == img_shape, patches_ref[-1].shape == lbl_shape)
...
print(patches_tr_ref[0].dtype, patches_val_ref[0].dtype, patches_test_ref[0].dtype)
print(np.max(patches_tr_ref[0]), np.max(patches_val_ref[0]))
...
# im = ax1.imshow(mean_prob, cmap =cmap)
# ax1.axis('off')
